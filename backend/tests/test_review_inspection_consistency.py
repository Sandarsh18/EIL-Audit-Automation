...
def test_review_inspection_consistency():
    ...
    # 3. VERIFY INSPECTION DAYS (7 days in August)
    ...
    assert calc_res_7["inspection"] == 24.0 # 3 days from July!
    
    # 6. RE-RUN REVIEW
    resp_rev_7 = client.post(f"/api/sessions/{s_id}/review", json={"job_numbers": ["B269"]})
    rev_res_7 = resp_rev_7.json()[0]
    assert rev_res_7["inspection"] == 24.0 # MUST MATCH COMBINED AND 2026-07 MONTH!
