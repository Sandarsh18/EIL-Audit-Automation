...
def test_combined_full_and_zero(setup_combined_session):
    ...
    assert j["inspection"] == 24.0 # 3 days * 8
    assert j["others"] == 0.0
    ...
    assert z["inspection"] == 8.0 # 1 days * 8
    assert z["others"] == 0.0
...
def test_combined_blocked_states(setup_combined_session):
    ...
    assert no_ocs["calculated_total"] == 30.0 # 6 + 24 + 0
    ...
    assert no_oth["calculated_total"] == 30.0 # 6 + 24 + 0
...
def test_combined_manual_overrides(setup_combined_session):
    ...
    assert ovr["inspection"] == 24.0 # 3 days * 8
