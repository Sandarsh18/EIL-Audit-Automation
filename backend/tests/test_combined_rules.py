...
def test_combined_full_and_zero(setup_combined_session):
    ...
    assert j["inspection"] == 3.0
    assert j["others"] == 0.0
    assert j["calculated_total"] == 9.0 # 6 + 3 + 0
    ...
    assert z["inspection"] == 1.0
    assert z["others"] == 0.0
    assert z["calculated_total"] == 1.0
...
def test_combined_blocked_states(setup_combined_session):
    ...
    assert no_ocs["calculated_total"] == 9.0
    ...
    assert no_oth["calculated_total"] == 9.0
...
def test_combined_manual_overrides(setup_combined_session):
    ...
    assert ovr["inspection"] == 3.0
    assert ovr["others"] == 10.0
    assert ovr["calculated_total"] == 29.0 # 16 + 3 + 10
